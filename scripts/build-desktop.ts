...
/**
 * NSIS customization, written where electron-builder looks for it by
 * convention: `<buildResources>/installer.nsh`, which it includes ahead of its
 * own installer script. The directory is excluded from the packaged files
 * automatically, so nothing here reaches the application tree.
 *
 * The `perMachine` option cannot express "always install for this user".
 * electron-builder defines `INSTALL_MODE_PER_ALL_USERS` only for
 * `perMachine: true`, and the assisted installer shows its "who is this for?"
 * page precisely when that macro is *absent* — so `perMachine: false` means
 * "let the person choose", not "per user". Two answers across two releases
 * leave two installations side by side, one under Program Files and one under
 * Local\Programs, neither of which the other's update overwrites.
 *
 * `customInstallMode` is the seam electron-builder provides for this: the
 * page's own pre-callback reads `$isForceCurrentInstall`, selects the per-user
 * mode, and aborts — and aborting a custom page's pre-callback is what skips
 * the page. The registry read is what keeps an existing machine-wide
 * installation reachable: forcing per-user on a machine that has one would
 * strand it beside a second copy rather than replace it, so that machine alone
 * still gets the choice.
 *
 * `customCheckAppRunning` replaces electron-builder's own routine, which
 * cannot be relied on to close a running app. Its per-user branch filters
 * `taskkill` with `/fi "USERNAME eq %USERNAME%"`, expanded through `cmd.exe`,
 * while the matching `tasklist` result is decided by a `find` on the image
 * name alone. The two disagree whenever that filter fails to match — a
 * non-ASCII account name is the ordinary way — and the installer is then
 * certain the app is running and unable to kill it, which is the
 * "…无法关闭，请手动关闭它" dialog with no way forward. This kills by image
 * name only, gently first and then with the process tree, and never stops to
 * ask. The app already exits on its own before starting the installer (see
 * `update-flow.ts`), so this path is what a manually launched installer
 * depends on, where no application code participates at all.
 *
 * TODO: the diagnostic half of `customCheckAppRunning` — the log written to
 * `%TEMP%\\dsh-install-log.txt` — is temporary. 0.1.7 still stopped on the
 * retry dialog, and that dialog has a second source: `extractAppPackage.nsh`
 * raises the same message after `CopyFiles` into the install directory fails
 * five times, which a still-running process explains but so does a directory
 * this installer cannot write. The log records which one it is — the install
 * mode and directory, whether the installer is elevated, whether that
 * directory accepts a file, both taskkill exit codes, and what still runs
 * afterwards. Remove it once the cause is known.
 *
 * That log answered it: the directory is writable and no process survives, so
 * neither earlier theory held. The failure is upstream of both — installing
 * over an existing version runs the *old* uninstaller first, and
 * `installUtil.nsh` treats any non-zero exit from it as fatal, showing
 * `uninstallFailed` and quitting. A partial uninstall is not worth abandoning
 * an upgrade over: the files it left behind are the ones this install is about
 * to overwrite. `customUnInstallCheck` replaces that judgement, recording the
 * exit code and letting the install proceed.
 */
const NSIS_CUSTOMIZATION = `# Generated by scripts/build-desktop.ts — edit that script, not this file.
...
!macro customCheckAppRunning
  System::Store S
  StrCpy $R9 "$TEMP\\dsh-install-log.txt"
  FileOpen $R8 "$R9" w
  FileWrite $R8 "=== DeepSeek Harness install diagnostic ===$\\r$\\n"
  FileWrite $R8 "INSTDIR=$INSTDIR$\\r$\\n"
  FileWrite $R8 "installMode=$installMode$\\r$\\n"
  \${if} \${UAC_IsAdmin}
    FileWrite $R8 "admin=yes$\\r$\\n"
  \${else}
    FileWrite $R8 "admin=no$\\r$\\n"
  \${endIf}
  ClearErrors
  CreateDirectory "$INSTDIR"
  FileOpen $R7 "$INSTDIR\\dsh-write-test.tmp" w
  \${if} \${Errors}
    FileWrite $R8 "instdir_writable=NO$\\r$\\n"
  \${else}
    FileClose $R7
    Delete "$INSTDIR\\dsh-write-test.tmp"
    FileWrite $R8 "instdir_writable=yes$\\r$\\n"
  \${endIf}
  DetailPrint "Closing \${PRODUCT_NAME}..."
  nsExec::Exec '"$SYSDIR\\taskkill.exe" /im "\${APP_EXECUTABLE_FILENAME}"'
  Pop $R6
  FileWrite $R8 "taskkill_soft_rc=$R6$\\r$\\n"
  Sleep 1500
  nsExec::Exec '"$SYSDIR\\taskkill.exe" /f /t /im "\${APP_EXECUTABLE_FILENAME}"'
  Pop $R6
  FileWrite $R8 "taskkill_force_rc=$R6$\\r$\\n"
  Sleep 500
  FileWrite $R8 "--- tasklist after kill ---$\\r$\\n"
  FileClose $R8
  nsExec::Exec '"$SYSDIR\\cmd.exe" /c tasklist /FI "IMAGENAME eq \${APP_EXECUTABLE_FILENAME}" /V /FO list >> "$R9"'
  DetailPrint "Diagnostic written to $R9"
  System::Store L
!macroend

!macro customUnInstallCheck
  StrCpy $R9 $R0
  FileOpen $R8 "$TEMP\\dsh-install-log.txt" a
  FileWrite $R8 "old_uninstaller_rc=$R9$\\r$\\n"
  FileClose $R8
  DetailPrint "Old uninstaller exit code: $R9 - continuing."
!macroend
`

/**
 * Write the electron-builder configuration, and the NSIS customization it
 * relies on, into the staged closure.
 *
...
`
  ...
  await writeFile(join(STAGING, 'build', 'installer.nsh'), NSIS_CUSTOMIZATION)
}
