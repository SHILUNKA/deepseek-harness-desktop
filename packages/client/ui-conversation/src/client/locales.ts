...
export const zh = {
  ...
  'promptError.agentBusy': '当前会话正在运行，请等它结束，或先点停止再发送',
  'promptError.sessionMissing': '这个会话已不存在，可能已被删除，请新建会话',
  'promptError.conflict': '会话内容已在别处变动，请刷新页面后重试',
  'promptError.cancelled': '请求已取消',
  'promptError.backendUnavailable': '后台服务未就绪，请稍等几秒重试；若一直如此请重启软件',
  'promptError.internal': '软件内部出错，请重试；若反复出现请把错误码反馈给我们（{code}）',
  'promptError.unknown': '发送失败，请重试；若反复出现请把错误码反馈给我们（{code}）',
...
export const en = {
  ...
  'promptError.agentBusy': 'This session is still running — wait for it to finish, or stop it first',
  'promptError.sessionMissing': 'This session no longer exists; start a new one',
  'promptError.conflict': 'The session changed elsewhere; reload the page and try again',
  'promptError.cancelled': 'Request cancelled',
  'promptError.backendUnavailable': 'The backend is not ready — retry in a few seconds, or restart the app',
  'promptError.internal': 'Something went wrong inside the app. Retry, and report this code if it persists ({code})',
  'promptError.unknown': 'Could not send. Retry, and report this code if it persists ({code})',
