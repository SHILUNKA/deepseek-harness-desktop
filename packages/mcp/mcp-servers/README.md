---
description: "The settings-managed MCP server host for users and maintainers adding or removing external tool servers without editing a patch file."
...
- The discarded design wrote patch rows through an API and a UI over them. Since an MCP server *is* a plugin instance, letting one managing plugin hold them is far simpler than teaching a settings surface to edit a patch file, and it reuses the existing settings and card machinery.
- Reconciliation leaves an entry whose command is unchanged running; a changed one is disposed with `await` before the replacement mounts. The name is released synchronously — what the `await` buys is that the *old child process* exits first, since an MCP server may hold a port, a lock, or a device.
- Only stdio is supported. `env`, `cwd`, and HTTP transports still need a composed `mcp-client` row. Connection state is not reported here; read it from the Loader's plugin inventory.
